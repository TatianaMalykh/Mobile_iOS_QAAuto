...
When(/^Проверяем наличие элемента "([^"]*)" класса "([^"]*)" с индексом "([^"]*)", вложенного в элемент id "([^"]*)"$/) do |name,my_class,index, id_name|
  if exist_element_class?("class:#{my_class}","index:#{index}","id:#{id_name}")
    puts ("Есть элемент #{name}.")
  else
    raise "Нет элемента #{name}!"
  end
...
When(/^Проверяем, что в элементе "([^"]*)" класса "([^"]*)" с индексом "([^"]*)", вложнного в элемент с id "([^"]*)", есть текст$/) do |name, my_class,index, id|
  ...
  if element[index].text?
    puts "В элементе есть текст #{name.text}"
  else
